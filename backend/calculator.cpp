// ════════════════════════════════════════════════════════════════
//  Reinforced-Concrete Column Fire-Resistance — C++17 backend FINAL ALL AS
...
static double gammaConcrete(double t) {
    // FINAL LOGIC:
    // Rbn өсөхөд гал тэсвэрлэх хугацаа өсөх боловч бетон өндөр температурт
    // хурдан сулардаг тул 22→29 болоход хугацаа хиймлээр 300+ мин болохгүй.
    // Энэ хүснэгт нь As1-only гараар бодсон ~117 мин логиктой тааруулсан
    // бөгөөд 500–700°C дээр бетоны үлдэх бат бэхийг огцом бууруулна.
    static const std::vector<std::pair<double,double>> pts = {
        {20, 1.00}, {100, 0.95}, {200, 0.86}, {250, 0.66}, {300, 0.58},
        {400, 0.42}, {500, 0.25}, {600, 0.10}, {700, 0.04},
        {800, 0.01}, {900, 0.00}, {1200, 0.00}
    };
    return interp(t, pts);
}
...
static StepResult computeStep(double tau,
    ...
    if (tau > 0.0) {
        ...
        // δ — гараар боддог логиктой ижил: хэт conservative болгож өсгөхгүй.
        // Бетоны температурын бууралт γ_b,tem-г тусад нь Rbn дээр үржүүлнэ.
